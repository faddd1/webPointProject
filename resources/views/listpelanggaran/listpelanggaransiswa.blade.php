    ...
    <div class="col-lg-12">
        <div class="card">
            <div class="card-header align-items-center">
                <h4 class="card-title text-md" style="margin-top: 5px;">Halo, <span class="text-bold">{{ Auth::user()->siswa->nama }}</span>!</h4>
                <div class="card-tools">
                    <span class="me-2">Sisa Poin :</span>
                    @if (Auth::user()->siswa->point <= 10)
                        <a class="btn btn-success btn-sm" style="color: #fff;">{{ Auth::user()->siswa->point }}/200</a>
                    @elseif (Auth::user()->siswa->point <= 50)
                        <a class="btn btn-primary btn-sm" style="color: #fff;">{{ Auth::user()->siswa->point }}/200</a>
                    @elseif (Auth::user()->siswa->point <= 100)
                        <a class="btn btn-warning btn-sm" style="color: #fff;">{{ Auth::user()->siswa->point }}/200</a>
                    @elseif (Auth::user()->siswa->point <= 200)
                        <a class="btn btn-danger btn-sm" style="color: #fff;">{{ Auth::user()->siswa->point }}/200</a>
                    {{-- @elseif (Auth::user()->siswa->point <= 200)
                        <a class="btn btn-danger btn-sm" style="color: #fff;">{{ Auth::user()->siswa->point }}/200</a> --}}
                    @endif
                </div>
